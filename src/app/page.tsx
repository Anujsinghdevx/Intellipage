...
import { NewDocument, ErrorBoundary } from "@/components";
...
const DevHealthCheck = () => {
  ...
  const { data, loading, error } = useFetch(isDev ? "/api/health" : null);

  if (!isDev) return null;

  return (
    <div className="fixed bottom-4 right-4 z-[60] rounded-md border border-gray-300 bg-white/90 px-3 py-2 text-xs text-gray-800 shadow-lg">
      {loading && "Checking API..."}
      {error && "API check failed"}
      {data && (data as { status?: string }).status === "ok" && "API healthy"}
    </div>
  );
};
